...
const { turnOnDevice, turnOffDevice, extractDayMonthYear, extractHourMinute } = require('../smartapp');
...
describe('turnOnDevice', () => {
  ...
  test('should execute "switch.on" command', async () => {
    ...
    await turnOnDevice(context, deviceId);
...
describe('turnOffDevice', () => {
  // Test 2
    test('should execute "switch.off" command', async () => {
      ...
      await turnOffDevice(context, deviceId);
